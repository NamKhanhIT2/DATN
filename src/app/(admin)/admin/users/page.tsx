...
export default function AdminUsersPage() {
  ...
  const { data, isLoading } = trpc.admin.getUsers.useQuery({ limit: 20 },{ staleTime: 1000 * 60 * 3, gcTime: 1000 * 60 * 5, });
                ...
                <TableRow key={user.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <TableCell className="text-right">
                    <Button
                      ...
                      onClick={() => {
                        if (confirm(`🛑 SYSTEM SECURITY:\nAre you sure you want to BAN the account of "${user.name}"?\nThe user will be kicked out immediately and cannot log back into the Tammi platform.`)) {
                          toggleBanMutation.mutate({ clerkId: user.clerkId, isBanned: true });
                        }
                      }}
                    >
